...
describe(App, () => {
  ...
  it("calls renderQuestions for each question in the state", () => {
    const questions = component.state("questions");
    const spy = jest.spyOn(App.prototype, "displayQuestion");
    component.instance().forceUpdate(); // Force the update to trigger the render loop
    expect(spy).toHaveBeenCalledTimes(questions.length);
  });
